...
describe( 'MarkdownEditor (block) build', () => {
	...
	describe( 'build', () => {
		it( 'contains plugins', () => {
			expect( MarkdownEditor.builtinPlugins ).to.not.be.empty;
		} );

		it( 'contains config', () => {
			expect( MarkdownEditor.defaultConfig.toolbar ).to.not.be.empty;
		} );

		it( 'contains block toolbar config', () => {
			expect( MarkdownEditor.defaultConfig.blockToolbar ).to.not.be.empty;
		} );
	} );

	describe( 'create()', () => {
		beforeEach( () => {
			return MarkdownEditor.create( editorElement )
				.then( newEditor => {
					editor = newEditor;
				} );
		} );
		...
		it( 'creates an instance which inherits from the MarkdownEditor', () => {
			expect( editor ).to.be.instanceof( MarkdownEditor );
	...
	describeMemoryUsage( () => {
		testMemoryUsage(
			...
			() => MarkdownEditor.create( document.querySelector( '#mem-editor' ) ) );
	} );
} );
